"""
Pocket-Dentist — prompt library.

Every prompt used in the benchmark, as ``string.Template`` objects, exactly as
used to produce the results in the paper. Verbatim from the research harness
(``src/utils/prompt.py``); released unmodified for reproducibility.

Groups
------
Inference (what each evaluated VLM is shown):
    * ``classify_intraoral_condition_for_image`` — MetaDent classification (18 labels)
    * ``captioning_intraoral_condition``         — MetaDent captioning
    * ``vqa_answer_intraoral_condition``         — MetaDent VQA (answer a question)

LLM-as-judge / evaluation (used by the captioning scorer):
    * ``captioning_extraction_intraoral_condition`` — extract abnormalities from a caption
    * ``captioning_score_intraoral_condition``      — TP/FN/FP/TN vs. the reference

Curation (how the QA pairs were generated from expert meta-labels; see qa_pairs/):
    * ``translate_dent_json_zh2en``      — normalise source meta-labels to English
    * ``summary_intraoral_condition``    — meta-labels -> image description
    * ``classify_intraoral_condition``   — meta-labels -> 18-label set
    * ``vqa_intraoral_condition``        — meta-labels -> VQA questions
    * ``verifier_intraoral_condition``   — verify/repair generated VQA against meta-labels

Label taxonomy: ``label_desc_en`` (the 18 MetaDent conditions, C1–C18).

All evaluation runs use greedy decoding (``do_sample=False``) for determinism.
"""
